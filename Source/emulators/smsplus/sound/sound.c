...
int sms_sound_init(void)
{
    ...
    snd.output[0] = malloc(snd.buffer_size*2);
...
void sms_sound_mixer_callback(int16 **stream, int16 **output, int length)
{
    int i;
	int outputcount;
    for(i = 0, outputcount=0; i < length; i++,outputcount+=2)
    {
        output[0][outputcount] = (fm_buffer[0][i]<<2) + psg_buffer[0][i];
        output[0][outputcount+1] = (fm_buffer[1][i]<<2) + psg_buffer[1][i];
    }
}
