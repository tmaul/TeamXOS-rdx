...
//	SetCurrentDirectory(Gens_Path);
	...
	if ((!stricmp("ZIP", &Name[strlen(Name) - 3])) || (!stricmp("ZSG", &Name[strlen(Name) - 3])))
	{
		...
		while (i == UNZ_OK)
		{
			...
			if ((int) zinf.uncompressed_size > filesize)
			{
				strcpy(filename,zname);
				filesize = zinf.uncompressed_size;
			}
			/*
		...
		//if (i) return 0;
		if (unzLocateFile(zf, filename, 1) != UNZ_OK) return 0;
...
Rom *Load_Rom_Zipped(HWND hWnd, char *Name, int inter)
{
	...
	while (Current == UNZ_OK)
	{
		...
		if ((int) Infos.uncompressed_size > filesize)
		{
			strcpy(filename,File_Name);
			filesize = Infos.uncompressed_size;
		}
	...
	if( !(Current == UNZ_END_OF_LIST_OF_FILE || Current == UNZ_OK) || filesize == 0)
...
//		MessageBox(hWnd, "No genesis or 32X roms file found in zip\n", "Error", MB_OK);
	...
	if (unzLocateFile(Rom_File, filename, 1) != UNZ_OK)
	{
		unzClose(Rom_File);
		Game = NULL;
		return NULL;
	...
	memset(Rom_Data, 0, 6 * 1024 * 1024);
	bResult = unzReadCurrentFile(Rom_File, Rom_Data, filesize);

	unzCloseCurrentFile(Rom_File);


	if ((bResult <= 0) || (bResult != filesize))
	{
